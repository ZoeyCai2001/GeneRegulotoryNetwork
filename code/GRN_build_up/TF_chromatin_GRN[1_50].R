...
#counts: large list
#counts$'Gene Expression':36601 x 11909 sparse Matrix of class "dgCMatrix"
#columns: AAACAGCCAAGGAATC-1;rows:genes
#counts$Peaks:108377 x 11909 sparse Matrix of class "dgCMatrix"
#columns: AAACAGCCAAGGAATC-1;rows:peaks
...
for(gene in gene_names){
  ...
  #131 objects: atac[1]="chr1-777634-779926"
  atc=pbmc.atac@assays$ATAC@counts[atac,]
  #131 x 11909 sparse Matrix of class "dgCMatrix"
  
  wh=which(M1[atac[1],]!=0)
  TF=(colnames(M1))[wh]
  #313 objects: TF in M1 where TF-atac[1]!=0

  #if length(w)=which(hvg[,gene]!=0)==1
  #only one atac in rownames(hvg1) has hvg[atac,gene]!=0
  if(length(w)==1){
    if(length(wh)==1){
      y=1
  ...
  if(length(w)>1){
    ...
    #more than one TF in M1 has M1[TF,peaks]!=0
    if(length(wh)>1){
      rna=pbmc.rna@assays$RNA@counts[TF,]
      for(k in 1:length(wh)) {
        rna[k,]=rna[k,]*atc[1,]
  ...
  write.table(result[1,],file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_51_100.csv",append = T,col.names = F,row.names = F,sep=",")
  ...
  for(i in 2:nrow(result)){
    ...
    if(!(a[[1]][1] %in% TFs)){ 
      ...
      TFs=c(TFs,a[[1]][1] )
      write.table(result[i,],file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_51_100.csv",append = T,col.names = F,row.names = F,sep=",")  }
    
  }
  
}
t=read.table("/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_51_100.csv",header = F,sep=',')
colnames(t) = c("Gene1","Gene2","Edgeweight")
write.table(t,file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_51_100.csv",row.names = F,sep=",")
...
#TF 对应weight相加
t=read.table("/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_51_100.csv",header = T,sep=',')

...
for(gene in gene_names){
  ...
  write.table(result,file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/TF__filename_unnormalization.csv",append = T,col.names = F,row.names = F,sep=",")
  #注意读取的时候header=F
}
t=read.table("/gpfs/gibbs/pi/zhao/zc354/GRN/output/TF__filename_unnormalization.csv",header = F,sep=',')
colnames(t) = c("Gene1","Gene2","Edgeweight")  #与Beeline保持一致
write.table(t,file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/TF__filename_unnormalization.csv",row.names = F,sep=",")

...
for(gene in gene_names[451:500]){
  y=0 #用于命名
  w=which(hvg1[,gene]!=0)
  ...
  write.table(result[1,],file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_451_500.csv",append = T,col.names = F,row.names = F,sep=",")
  ...
  for(i in 2:nrow(result)){
    ...
    if(!(a[[1]][1] %in% TFs)){ 
      ...
      TFs=c(TFs,a[[1]][1] )
      write.table(result[i,],file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_451_500.csv",append = T,col.names = F,row.names = F,sep=",")  }
    
  }
  
}
t=read.table("/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_451_500.csv",header = F,sep=',')
colnames(t) = c("Gene1","Gene2","Edgeweight")
write.table(t,file="/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_451_500.csv",row.names = F,sep=",")
...
#TF 对应weight相加
t=read.table("/gpfs/gibbs/pi/zhao/zc354/GRN/output/multiply_filename_451_500.csv",header = T,sep=',')
library(plyr)

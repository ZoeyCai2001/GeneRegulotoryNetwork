...
new_list = np.append(thresholds_filter, 1)
DF = pd.DataFrame(prec_filter)
DF['recall']= recall_filter
DF['threshold']= new_list 
DF['positive']= 0
DF['true positive']= 0
DF.columns = ['precision','recall','thresholds','positive','true positive']

outDF_filter = outDF_filter.sort_values(by="Pred",ascending=False)
trueDF = outDF_filter.loc[outDF_filter['True'] > 0]
true = set(trueDF.index.values)
print('filter_begin')
for i in range(len(prec_filter)):
    newDF = outDF_filter.loc[outDF_filter['Pred'] >= DF.loc[i,'thresholds']]
...
DF.to_csv("/gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_filter_bmmc_500kb.csv",index=False,sep=',')

print("filter end and saved as /gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_filter_bmmc_500kb.csv")
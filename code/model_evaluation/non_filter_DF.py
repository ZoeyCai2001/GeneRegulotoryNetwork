...
new_list = np.append(thresholds_non_filter, 1)
DF = pd.DataFrame(prec_non_filter)
DF['recall']= recall_non_filter
DF['threshold']= new_list 
DF['positive']= 0
DF['true positive']= 0
DF.columns = ['precision','recall','thresholds','positive','true positive']

outDF_non_filter = outDF_non_filter.sort_values(by="Pred",ascending=False)
trueDF = outDF_non_filter.loc[outDF_non_filter['True'] > 0]
true = set(trueDF.index.values)
print('non_filter_begin')
for i in range(len(prec_non_filter)):
    newDF = outDF_non_filter.loc[outDF_non_filter['Pred'] >= DF.loc[i,'thresholds']]
...
DF.to_csv("/gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_non_filter_pbmc_500kb.csv",index=False,sep=',')
print('non_filter end and saved as /gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_non_filter_pbmc_500kb.csv')  

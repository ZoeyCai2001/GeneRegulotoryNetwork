...
new_list = np.append(thresholds_multiply, 1)
DF = pd.DataFrame(prec_multiply)
DF['recall']= recall_multiply
DF['threshold']= new_list 
DF['positive']= 0
DF['true positive']= 0
DF.columns = ['precision','recall','thresholds','positive','true positive']

outDF_multiply = outDF_multiply.sort_values(by="Pred",ascending=False)
trueDF = outDF_multiply.loc[outDF_multiply['True'] > 0]
true = set(trueDF.index.values)
print('multiply_begin')
for i in range(len(prec_multiply)):
    newDF = outDF_multiply.loc[outDF_multiply['Pred'] >= DF.loc[i,'thresholds']]
...
DF.to_csv("/gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_multiply_bmmc_500kb.csv",index=False,sep=',')
print("multiply end and saved as /gpfs/gibbs/pi/zhao/zc354/GRN/output/topk_multiply_bmmc_500kb.csv")